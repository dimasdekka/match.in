...
import { WelcomeScreen, OnboardingWizard } from './modules/onboarding';
import { DiscoverPage as BrandedDiscoverPage } from './modules/discover';
...
export const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const localProfile = window.localStorage.getItem('matchin:onboarding-profile');
    if (localProfile) {
    ...
    const loadAppData = async () => {
      ...
      try {
        ...
        const totalUnread = (convsRes.conversations || []).reduce(
  ...
  const handleCompleteOnboarding = (formData: ProfileFormData) => {
    window.localStorage.setItem('matchin:onboarding-profile', JSON.stringify(formData));
    setShowWelcome(false);
    setShowOnboarding(false);
  };
  ...
  if (!showWelcome && !showOnboarding) {
    return <BrandedDiscoverPage />;
  }

  return (
    <div className="match-app min-h-screen flex flex-col selection:bg-pink-500 selection:text-white">
      ...
      {showOnboarding && (
        <OnboardingWizard initialName={initialTelegramName} onComplete={handleCompleteOnboarding} />
      )}

      {/* Header — hidden during welcome/onboarding */}
      {!isFullscreenOverlay && (
        <Header currentLang={currentLang} onLanguageChange={(lang) => setCurrentLang(lang)} />
      )}
      ...
      {!isFullscreenOverlay && (
        <main className="flex-1 flex flex-col relative overflow-y-auto pb-20">
                ...
                <div className="text-center space-y-4 max-w-xs">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 mt-1 leading-relaxed">
                      Kamu telah melihat semua rekomendasi di sekitarmu. Coba ubah filter lokasi
                      atau refresh kembali nanti!
                    </p>
                  </div>
                  <button
                    onClick={async () => {
          ...
          {/* Screen 5: Likes Tab */}
          {activeTab === 'likes' && <LikesPage onOpenMatchesCount={() => {}} />}
            ...
            <div className="w-full max-w-md mx-auto space-y-3 px-4 pb-24 animate-fade-in">
              ...
              {chatsLoading ? (
                ...
                conversations.map((conv) => {
                  ...
                  try {
                    photos =
                      typeof prof.photos === 'string' ? JSON.parse(prof.photos) : prof.photos || [];
                  } catch {
                    photos = [];
                  }
                  const avatar =
                  ...
                  const lastMsg =
                    conv.last_message?.content || 'Match baru! Mulai obrolan sekarang 💕';
      ...
      {activeChatProfile && (
        <ChatModal matchedProfile={activeChatProfile} onClose={() => setActiveChatProfile(null)} />
      )}
